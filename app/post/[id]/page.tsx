...
import { dislikePost } from "@/app/utils/dislikePost";
import { undislikePost } from "@/app/utils/undislikePost";
...
const PostPage = ({ params }: PostPageProps) => {
  ...
  // useEffect for getting the post information
  useEffect(() => {
    if (!id) return;
  ...
  // getting the subcollection of users that disliked the post
  useEffect(() => {
    const dislikedPostRef = collection(db, "posts", id, "dislikes");

    //real-time listener
    const unsubscribe = onSnapshot(dislikedPostRef, (querySnapshot) => {
      const userIds = querySnapshot.docs.map((doc) => doc.id);
      if (firestoreUser) {
        setIsDisliked(userIds.includes(firestoreUser.uid));
      }
    });
    return () => unsubscribe();
  }, [id, firestoreUser]);
      ...
      {post && (
        <div className="font-sans flex flex-col p-8 pb-20 gap-8 sm:p-20 w-full ">
          <div className="bg-neutral text-neutral-content shadow-xl overflow-hidden rounded-xl ">
            ...
            {/* DISPLAYING LIKE BUTTON ON POSTS */}
            {firestoreUser && (
              <div className="flex gap-4 px-6 py-4">
                ...
                <button
                  onClick={() => {
                    if (isDisliked) {
                      undislikePost(firestoreUser!.uid, id);
                    } else {
                      dislikePost(firestoreUser!.uid, id);
                      if (isLiked) {
                        unlikePost(firestoreUser!.uid, id);
                      }
                    }
            ...
            {/* DISPLAYING COMMENT BUTTON ON POSTS */}
            {firestoreUser && (
                ...
                <li key={comment.commentId} className="flex p-4">
                  <div>
                    <div className="flex items-center pb-4">
                      ...
                      <div>
                        <p className="pl-2 text-lg xl:text-2xl">
                          {comment.userName}
                        </p>
